...
public final class S3PostSigner {
   ...
   * href="https://docs.aws.amazon.com/general/latest/gr/sigv4_signing.html">Aws Signature Version 4
   * specification</a> <br>
   * <br>
  ...
  public static PresignedPost create(
    ...
    return new PresignedPost(
        ...
        createConditionsMap(
            credentials, signature, amzDate, policyB64, keyUploadValue, returnConditions));
  }
   ...
   * S3 Post Policy</a>. This method might be useful though for using new features made available by
   * AWS not yet added to {@link #create(PostParams, AwsCredentialsProvider)} or for troubleshooting
   * using raw data. For reference about how to use this method, check the correspondent integration
   * tests in the source code. <br>
   * <br>
   ...
   *     pre signed post. Note that these fields only are not enough to perform the file upload. The
   *     caller must add the other necessary fields matching the conditions passed to this method.
   */
  ...
  /**
   * Removes the {@link ConditionField#CONTENT_LENGTH_RANGE} and {@link ConditionField#BUCKET} since
   * they are not necessary to be added in the client using the pre signed post.
   *
   ...
   * returned. Otherwise, a "start with" condition was used and therefore is not possible to foresee
   * which value will be used by the pre signed post caller
   *
   * @return The exact value to be used by the pre signed post caller or an empty string indicating
   *     that the caller has to provide the data themselves.
   */
  ...
  private static String generateSignature(
    ...
    return AwsSigner.hexDump(
        AwsSigner.signMac(
            AwsSigner.generateSigningKey(awsCredentials.secretAccessKey(), region, amzDate),
  ...
  private static void addSessionTokenIfNeeded(
    ...
    if (awsCredentials instanceof AwsSessionCredentials) {
      ...
      conditions.put(
          SECURITY_TOKEN,
          new MatchCondition(
              SECURITY_TOKEN, EQ, ((AwsSessionCredentials) awsCredentials).sessionToken()));
    }
  }
  ...
  private static Set<String[]> buildConditions(
    ...
    result.add(
        new String[] {EQ.awsOperatorValue, DATE.valueForAwsPolicy, xAmzDate.formatForPolicy()});
