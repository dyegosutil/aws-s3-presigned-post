...
public class PostParams {
   ...
   *     specified by AWS.
   * @param exactKeyCondition Specifies which is the exact value that should be used to perform the
   *     upload. For convenience, use the {@link KeyConditionHelper#withKey(String)} to build the
   *     condition.
  ...
  public static final class Builder {
...

    {
      ...
      dependentConditionFields.put(
          SERVER_SIDE_ENCRYPTION_CONTEXT, new TreeSet<>(singletonList(SERVER_SIDE_ENCRYPTION)));
      dependentConditionFields.put(
      ...
      dependentConditionFields.put(
          ...
          new TreeSet<>(
              asList(
                  SERVER_SIDE_ENCRYPTION_CUSTOMER_ALGORITHM, SERVER_SIDE_ENCRYPTION_CUSTOMER_KEY)));
    }

    private Builder(
        Region region, AmzExpirationDate expirationDate, KeyCondition keyCondition, String bucket) {
      Objects.requireNonNull(region);
     ...
     * <br>
     * Some conditions require other conditions to be present for the pre signed post to be valid.
     * This method validates if said conditions where added, failing fast otherwise
     *
                        ...
                        if (!conditions.containsKey(requiredConditionField)) {
                          throw new IllegalArgumentException(
                              String.format(
                                  ...
                                  conditionField, requiredConditions));
                        }
                      });
                }
              });
    ...
    /**
     * Add tags if the method {@link Builder#withTag(String, String)} was used. Plain text is used
     * due to simplicity of xml and to avoid an overkill of using a library
     */
    ...
    /**
     * Makes sure that an exact ({@link MatchCondition.Operator#EQ}) mutually exclusive condition is
     * not added with a startsWith ({@link MatchCondition.Operator#STARTS_WITH}) condition.
     *
    ...
    /**
     * Makes sure that a startsWith ({@link MatchCondition.Operator#STARTS_WITH})mutually exclusive
     * condition is not added with an exact ({@link MatchCondition.Operator#EQ}) condition.
     *
     * @param conditionField Condition type specified
     * @param value Specifiled value for this condition
     * @return @return The {@link Builder} object
    ...
    private Builder assertUniquenessAndAdd(MatchCondition matchCondition) {
      return addIfUnique(
          matchCondition, getInvalidConditionExceptionMessage(matchCondition.getConditionField()));
    }
          ...
          .forEach(
              a -> {
                if (conditions.containsKey(a)) {
                  throw new IllegalArgumentException(
                      "Only one checksum condition CRC32, CRC32C, SHA1 or SHA256 can be added at"
                          + " the same time");
                }
              });
    ...
    private Builder assertUniquenessAndAddTagging(String value) {
      if (!tags.isEmpty()) {
        throw new IllegalArgumentException(
            "Either the method withTag() or withTagging() can be used for adding tagging, not"
                + " both");
      }
     ...
     *
     * <p>For example, calling the method with the values withContentLengthRange(1048576, 10485760)
     * allows the upload of a file from 1 to 10 MiB
     *
     * @param minimumValue Specified in bytes, indicates the minimum size of the file for it to be
     *     accepted
     * @param maximumValue Specified in bytes, indicates the maximum size of the file for it to be
     *     accepted
    ...
    /**
     * Used to define how should be the response code returned by AWS when the upload is successful.
     * The enum received as parameter here will be converted to 200, 201 or 204. Same values that
     * should be used while using the pre-singed post to upload the file.
     *
     * <p>If the value is set to OK (200) or NO_CONTENT (204), Amazon S3 returns an empty document
     * with a 200 or 204 status code.
     *
     * <p>If the value is set to CREATED (201), Amazon S3 returns an XML document with a 201 status
     * code:
     *
     * <pre>{@code
     ...
     * {@link PostParams.Builder#withSuccessActionRedirectStartingWith(String)}, the status code 204
     * is returned by default to the client when the upload succeeds.
     *
     ...
     * 'audio/aac', 'text/plain'. This can be seen in the metadata information in the s3 bucket. Not
     * to be confused with file extension. To limit that use {@link KeyConditionHelper}
     *
    ...
    /**
     * Allows specifying how should be the beginning of the content type for this upload. Example:
     * 'audio/aac', 'text/plain'. This can be seen in the metadata information in the s3 bucket. Not
     * to be confused with file extension. To limit that use {@link KeyConditionHelper}
     *
     * @param value Content Type to be used
    ...
    /**
     * Allows specifying which is the exact content disposition of the file being uploaded. Example:
     * 'inline', 'attachment'
     *
    ...
    /**
     * Allows specifying which is the exact content encoding of the file being uploaded. Examples:
     * 'gzip', 'compress', etc
     *
    ...
    /**
     * Allows specifying which is the exact Expires condition of the file being uploaded. Example:
     ...
     * uploaded. <br>
     * <br>
     * This condition is used to redirect the user to another page after the upload. AWS will add
     * query parameters into the end of the url such as {@code
     ...
     * <br>
     * This condition is used to redirect the user to another page after the upload. AWS will add
     * correspondent query parameters into the end of the url such as {@code
     * https://www.mydomain.com/?bucket=mybucket&key=test.txt&etag=%2254b0c58c7ce9f2a8b551351102ee0938%22}
     * <br>
     * <br>
     *
     * @param value success_action_redirect condition value to be added to the policy, ex: {@code
     *     https://www.mydomain.com}
     * @return The {@link Builder} object
     */
     ...
     * recommends that you disable ACLs except in unusual circumstances where you need to control
     * access for each object individually.</a>.
     *
     * <p>Allows specifying how should be the acl this object.
     *
     * <p>Check <a
     * href="https://docs.aws.amazon.com/AmazonS3/latest/userguide/acl-overview.html">this</a> link
     * to understand what each one of the canned acl permissions mean.
     */
    ...
    /**
     * Allows specifying which tags should be added for the file being uploaded. If this method is
     * used then the {@link Builder#withTag(String, String)} cannot. The value should be in the
     * follow format:
     *
     * <pre>{@code
     * <Tagging>
    ...
    /**
     * Allows specifying a tag to be added for the file being uploaded. If this method is used then
     * the {@link Builder#withTagging(String)} cannot.
     *
    ...
    /**
     * Allows the user to specify freely their own meta-data for the file being uploaded. It can be
     * used multiple times. Aws s3 does not validate any user meta-data. It stores user-defined
     * metadata keys in lowercase. Find more information <a
     ...
     * being uploaded. It can be used multiple times. Aws s3 does not validate any user meta-data.
     * It stores user-defined metadata keys in lowercase. Find more information <a
     * href="https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingMetadata.html">here</a>
     *
     * @param metaName The name of this meta-data
     ...
     * documentation</a>
     *
     * @param value An object in the same bucket or website to redirect to. Ex: '/anotherPage.html',
     *     'www.example.com/'
     * @return The {@link Builder} object
     */
    ...
    /**
     * Allows specifying the encryption context for this file upload. That is, and optional set of
     * key-value pairs that can contain contextual information about the upload. It is used as ADD,
     * that is, non-secret data that is provided to encryption and decryption operations to add an
     * additional integrity and authenticity check on the encrypted data. For more information vide
     * 'Encryption context' in the <a
     ...
     * encryption is used to encrypt new objects in this bucket, the bucket key reduces encryption
     * costs by lowering calls to AWS KMS. More information <a
    ...
    /**
     * Allows specifying the base64 encoded encryption key to be used for this file upload. The key
     * must be appropriate for use with the algorithm specified. For example, using a 265 bit
    ...
    /**
     * Allows specifying the base64 encoded 128-bit MD5 digest of the encryption key. Amazon S3 uses
    ...
    // To configure the POST policy to allow any content within a form field, use starts-with with
