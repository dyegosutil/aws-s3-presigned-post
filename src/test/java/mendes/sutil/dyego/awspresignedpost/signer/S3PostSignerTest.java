...
public class S3PostSignerTest {
  ...
  void shouldReturnCorrectFreeTextPreSignedPost() {
    ...
    conditions.add(
        new String[] {
          ...
          AWS_FAKE_KEY
              + "/"
              + getYyyyMmDdDateFormatter().format(date)
              + "/"
              + region
              + "/s3/aws4_request"
        });
    ...
    PresignedFreeTextPost preSignedPost =
        S3PostSigner.create(
            freeTextPostParams, getAmazonCredentialsProvider(AWS_FAKE_KEY, AWS_FAKE_SECRET));
  ...
  void shouldValidateAwsCredentialsForPreSignedPost() {
    assertThatThrownBy(
            () ->
                S3PostSigner.create(
                    createPostParamsWithKeyStartingWith(), mockAwsCredentialsProvider()))
  ...
  void shouldValidateAwsCredentialsForFreeTextPreSignedPost() {
    assertThatThrownBy(
            () -> S3PostSigner.create(createFreeTextPostParams(), mockAwsCredentialsProvider()))
        .isInstanceOf(NullPointerException.class)
        .hasMessage(ERROR_MESSAGE_NULL_AWS_CREDENTIALS);
  }
