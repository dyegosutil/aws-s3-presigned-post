...
class FreeTextPostParamsTest {
  ...
  void shouldThrowExceptionWhenUsingNullArgumentsCreatingFreeTextPostParams(
      String testCaseName, Supplier<FreeTextPostParams> supplier) {
    ...
    assertThatThrownBy(
            () ->
                new FreeTextPostParams(
                    Region.AP_EAST_1, ZonedDateTime.now(), ZonedDateTime.now(), new HashSet<>()))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  public static Stream<Arguments> getTestCases() {
    return Stream.of(
        of(
            ...
            (Supplier<FreeTextPostParams>)
                () ->
                    new FreeTextPostParams(
                        null, ZonedDateTime.now(), ZonedDateTime.now(), new HashSet<>())),
        of(
            "Should throw an IllegalArgumentException when expirationDate is null",
            (Supplier<FreeTextPostParams>)
                () ->
                    new FreeTextPostParams(
                        Region.AP_EAST_1, null, ZonedDateTime.now(), new HashSet<>())),
        of(
            "Should throw an IllegalArgumentException when date is null",
            (Supplier<FreeTextPostParams>)
                () ->
                    new FreeTextPostParams(
                        Region.AP_EAST_1, ZonedDateTime.now(), null, new HashSet<>())),
        of(
            "Should throw an IllegalArgumentException when conditions is null",
            (Supplier<FreeTextPostParams>)
                () ->
                    new FreeTextPostParams(
                        Region.AP_EAST_1, ZonedDateTime.now(), ZonedDateTime.now(), null)));
  }
}
