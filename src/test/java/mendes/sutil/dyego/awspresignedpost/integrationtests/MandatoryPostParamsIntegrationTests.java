...
public class MandatoryPostParamsIntegrationTests extends IntegrationTests {
   ...
   *     by the pre-signed post, which are either values for "startWith" conditions or wrong values
   *     for asserting failure
   */
  ...
  private PostParams createPostParamsWithExactKeyCondition(
      Region region, ZonedDateTime expirationDate, String bucket, KeyCondition keyCondition) {
    return PostParams.builder(region, expirationDate, bucket, castToExactKeyCondition(keyCondition))
        .build();
  }
  ...
  private PostParams createPostParamsWithKeyStartingWithCondition(
    ...
    return PostParams.builder(
            region, expirationDate, bucket, castToKeyStartingWithCondition(keyCondition))
        .build();
  }
  ...
  private static Stream<Arguments> getCustomizedUploadConditionsTestCases() {
    return Stream.of(
        // key
        of(
            "Should fail while uploading file to S3 using a different key then specified in the"
                + " policy",
            REGION,
            EXPIRATION_DATE,
            BUCKET,
        ...
        // bucket
        of(
            "Should fail while uploading file to S3 using a different bucket then the one"
        ...
        // region
        of(
            "Should fail while uploading file to S3 using a different region then the one"
                + " configured in the policy",
        ...
        // key starts-with
        of(
            "Should succeed while uploading file to S3 when key correctly starts-with the value"
        ...
        // key starts-with
        of(
            "Should fail while uploading file to S3 when key does not starts-with the value"
                + " specified in the policy",
            REGION,
            EXPIRATION_DATE,
            BUCKET,
            ...
            false),
        // key starts-with anything - used also when the file name provided by the user should be
        // used
        of(
            "Should succeed while uploading file to S3 when key correctly starts-with the value"
                + " specified in the policy",
